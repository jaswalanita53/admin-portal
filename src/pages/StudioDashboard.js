...
import { fatchStudioAvailableOccupiedHoldTodayfunction, fetchStudioDashboardDetails, fetchStudioRevenueOf14DaysMonthly, fetchstudioTomorrowDetails } from '../apiService/dashboard.js';
...
function StudioDashboard() {
  ...
  const getAvailableOccupiedHoldTodayfunction = async () => {
    ...
    try {
      const response = await fatchStudioAvailableOccupiedHoldTodayfunction(data);
  ...
  const getRevenueOf14DaysMonthly = async () => {
    const data = [];
    try {
      const response = await fetchStudioRevenueOf14DaysMonthly(data);
  ...
  const getTomorrowDetails = async () => {
    ...
    try {
      const response = await fetchstudioTomorrowDetails(data);
  ...
  const getDashboardDetails = async () => {
    ...
    try {
      const response = await fetchStudioDashboardDetails(data);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (location.pathname === "/studio/dashboard") {
        getDashboardDetails();
      }
    }, 30000);
